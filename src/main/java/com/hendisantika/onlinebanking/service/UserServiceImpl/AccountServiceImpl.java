...
public class AccountServiceImpl implements AccountService {
    ...
    @PostConstruct
    public void init() {
        PrimaryAccount lastPrimary = primaryAccountDao.findTopByOrderByAccountNumberDesc();
        SavingsAccount lastSavings = savingsAccountDao.findTopByOrderByAccountNumberDesc();

        int maxPrimary = (lastPrimary != null) ? lastPrimary.getAccountNumber() : 0;
        int maxSavings = (lastSavings != null) ? lastSavings.getAccountNumber() : 0;

        nextAccountNumber = Math.max(nextAccountNumber, Math.max(maxPrimary, maxSavings));
    }
